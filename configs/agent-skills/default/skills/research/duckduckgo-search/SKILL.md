...
- **Disambiguate plugin ecosystems**: queries like "Obsidian plugins" can mean plugins that run inside Obsidian, editor plugins for editing Obsidian vaults (e.g. Neovim), browser extensions, or MCP/agent bridges. If the user names another host app/editor (`nvim`, VS Code, browser, CLI), search for that host app plus the format/workflow (`Obsidian wikilinks`, `vault`, `Markdown wiki`) before assuming the native Obsidian community-plugin directory.
