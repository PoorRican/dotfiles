...
- If a single package must be newer than the primary pinned nixpkgs, prefer a narrow secondary nixpkgs input plus overlay/package reference over broad-updating the primary input. This preserves binary-cache hits and avoids source-building Rust/Node packages just because `overrideAttrs` changed hashes.
- For Rust packages, be cautious with source-only `overrideAttrs`: `rustPlatform.buildRustPackage` may retain computed `cargoDeps`/combined `patches`, so changing `version`/`src`/`cargoHash` alone can still apply stale `cargoPatches` or force long local builds. If the current nixpkgs branch already has the desired package, importing that package from a separate pinned nixpkgs input is usually safer.
...
- when creating local-only dotfiles automation that commits generated/agent skill changes, prefer a deterministic `~/.hermes/scripts/` script run by a Hermes `no_agent=true` cron job with `deliver='local'`. The script should use an explicit repo-relative path allowlist, exclude runtime bookkeeping like Hermes skill `.usage.json`/curator backups, delay if `git diff --cached` shows any pre-existing staged changes, and validate the staged set before committing. See `references/dotfiles-skill-autocommit-cron.md`.
...
- for Helix PKM/language-server setup in this dotfiles repo, keep editor-heavy LSP packages in a separate profile such as `nix/profiles/pkm.nix` and import it only on hosts that need it. Use the Helix language support table to distinguish true LSP-backed languages from syntax-only filetypes; do not invent packages for filetypes with no default server. Prefer `superhtml` for modern HTML support when asked, override Markdown to `markdown-oxide` if `marksman` is absent, override YAML to only `yaml-language-server` unless Ansible support is desired, and use `lib.hiPrio rust-analyzer` if `rustup` also provides a conflicting `bin/rust-analyzer`. Verify with `hx --health <language>` after `home-manager switch`. See `references/helix-pkm-language-server-profile.md`.
- for bun-installed coding agents or other global CLIs in this multi-host dotfiles repo, first confirm the package's npm `bin` and `engines` metadata, then check whether the pinned and upstream nixpkgs branches actually provide a new enough `pkgs.bun`. If not, prefer a minimal shared `pkgs.bun` overlay over broad lockfile churn: override `version`, all supported platform `passthru.sources`, and explicit `src`. Put the external install in a reusable Home Manager module/profile rather than a host-only hack when the tool is generally useful, and include `$HOME/.bun/bin` in activation/runtime PATH checks so `command -v <binary>` sees `bun install -g` outputs. Verify with all-host Home Manager evals plus a real `home-manager switch` and CLI smoke tests on the target host. See `references/bun-global-agent-overlay.md`.
- for Claude Code plugin/MCP cleanup in this dotfiles repo, check `configs/claude-code/settings.json` first; `nix/modules/claude-code.nix` deploys `~/.claude/settings.json` as an out-of-store symlink, so boolean changes under `enabledPlugins` affect new Claude/OMP sessions immediately once the file is saved, without a Home Manager switch. Preserve unrelated local edits in that JSON, validate with `python -m json.tool`, verify `readlink -f ~/.claude/settings.json`, and tell the user that existing agent sessions keep already-started MCP sidecars until closed.
...
- `references/dotfiles-skill-autocommit-cron.md` — local-only Hermes cron pattern for committing only skill-path changes from `~/dotfiles`, with staged-index delay semantics, path allowlisting, runtime-bookkeeping exclusions, and dry-run verification via a temporary git index.
...
- `references/helix-pkm-language-server-profile.md` — Helix PKM profile pattern: mapping Helix language-support rows to nixpkgs LSP packages, syntax-only filetype caveats, `superhtml`/`markdown-oxide` overrides, Rustup `rust-analyzer` conflict handling, and `hx --health` verification.
- `references/bun-global-agent-overlay.md` — pattern for bun-installed global coding agents when nixpkgs lags the required Bun version: package metadata checks, cross-platform source overlay, `$HOME/.bun/bin` activation PATH, shared module wiring, and runtime smoke tests.
