...
## Local/LAN hostname discovery

For home-network devices where the task is to recover router/DHCP DNS names (for example `host.home.local`), use the LAN workflow in `references/local-lan-name-discovery.md`:

- discover the active subnet and router DNS server;
- run a DNS-free ping/ARP sweep first (`nmap -sn -n <subnet>`);
- query the router DNS server directly for A and PTR records, especially when `.local` names are affected by mDNS/systemd-resolved behavior;
- verify by forward+reverse DNS consistency, reachability, and optionally MAC OUI/service checks.

When reporting local-device results, include FQDN, short name, IP address, and the verification basis.
